    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (!endApp)
            {
                ...
                if (op == null || !Regex.IsMatch(op, "^(a|s|m|d|sqrt|pow|10x|sin|cos|tg|h|c)$"))
                {
                ...
                else if (Regex.IsMatch(op, "^(sqrt|10x|sin|cos|tg)$"))
                {
                    ...
                    result = calculator.DoOperation(cleanNum, 0, op);
                    Console.WriteLine("Your result: {0:0.##}\n", result);
                }
                else if (op == "h")
                {
                    Console.WriteLine($"The number of expressions counted: {calculator.CountExpressions}");
                    Console.WriteLine("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                    foreach (var history in calculator.History)
                    {
                        Console.WriteLine($"Expression: {history.Expression}");
